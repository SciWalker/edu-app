#!/usr/bin/env python3
"""
Gemini-powered educational chatbot with LangGraph tools integration.
Provides conversational AI for educational queries with actual Google Classroom access.
"""
...
class GeminiChatbot:
    ...
    def __init__(self):
        ...
        if ChatGoogleGenerativeAI is None:
            raise ImportError("LangGraph dependencies not available. Please install: pip install langchain-google-genai langgraph")
    ...
    def _initialize_model(self):
        ...
        if not self.api_key:
            raise ValueError("No Gemini API key found. Please set GOOGLE_AI_STUDIO_API_KEY or add it to config.yml")
        ...
        print("✓ Gemini model with LangGraph tools initialized")
...
If users ask about their courses or classroom data, start by calling get_courses() to provide real information."""
    ...
    def export_conversation(self, file_path: str = None) -> str:
        ...
        if not file_path:
            ...
            file_path = f"conversation_export_gemini_tools_{timestamp}.json"
